  ...
  return (
    <section data-testid="journey-section" className="relative py-32 lg:py-44">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        >
          <p className="text-[#b31b1b] uppercase tracking-[0.35em] text-xs mb-6">No mystery, no jargon</p>
            ...
            {steps.map((s, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative flex gap-10 pl-16 lg:pl-0 lg:w-1/2 ${
                    i % 2 === 1 ? "lg:ml-auto lg:pl-16" : "lg:pr-16 lg:text-right lg:flex-row-reverse"
                  }`}
                >
                  <div
                    className={`absolute left-6 lg:left-auto top-1 -translate-x-1/2 w-12 h-12 flex items-center justify-center bg-[#161314] border border-[#d4af37]/40 shadow-[0_0_25px_rgba(212,175,55,0.25)] ${
                      i % 2 === 1 ? "lg:left-0" : "lg:-right-6 lg:translate-x-0"
                    }`}
                  >
                    <Icon size={20} className="text-[#d4af37]" />
                  </div>
                  <div>
                    <p className="text-[#a89f95] text-xs uppercase tracking-[0.3em] mb-3">Step {i + 1}</p>
                    <h3 className="font-display font-semibold uppercase text-3xl lg:text-4xl tracking-tight">
                      {s.title}
                    </h3>
                    <p className="mt-4 text-lg text-[#a89f95] leading-relaxed max-w-md">{s.text}</p>
                  </div>
                </motion.div>
              );
            })}
